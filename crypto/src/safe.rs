...
/// TODO: rename Safe to SafeArray once I add CipherText back as SafeBlock
// -> plan to define a codec structure for encrypting and backing SafeBlocks in SafePages
pub struct Safe<S: Size>(GenericArray<u8, S>);

impl<S: Size> core::fmt::Debug for Safe<S> {
    fn fmt(&self, f: &mut core::fmt::Formatter) -> core::fmt::Result {
        write!(f, "{}", std::any::type_name::<Self>())
    }
}

impl<S: Size> Drop for Safe<S> {
    fn drop(&mut self) {
        self.0.zeroize()
    }
}

impl<S: Size> AsRef<[u8]> for Safe<S> {
    fn as_ref(&self) -> &[u8] {
        self.0.as_ref()
    }
}

impl<S: Size> AsMut<[u8]> for Safe<S> {
    fn as_mut(&mut self) -> &mut [u8] {
        self.0.as_mut()
    }
}

impl<S: Size> PartialEq for Safe<S> {
    fn eq(&self, other: &Self) -> bool {
        self.as_ref().ct_eq(other.as_ref()).into()
    }
}

impl<S: Size> Eq for Safe<S> {}

impl<S: Size> Encode for Safe<S> {
...
mod tests {
    ...
    async fn test_encode_decode() {
        let key = Safe::<U32>::random().await;
        let key2 = Safe::<U32>::decode(&mut &key.encode()[..]).unwrap();
        assert_eq!(key, key2);
    }
}
